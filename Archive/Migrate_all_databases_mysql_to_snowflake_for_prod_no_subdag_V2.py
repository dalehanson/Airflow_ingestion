...
parent_dag_name = 'Migrage_all_databases_mysql_to_snowflake_for_prod_no_subdag_V2'
...
migration_audit_folder_path = 'migration_audit_files/'
scheduler_log_file_path = '/root/airflow/'
airflow_db_log_lifepan = 7
...
def start_audit_source_to_s3_v2(Source_System_Name, date_field, db_name, table_name, Task_id, source_connection, sf_connection
    ...
    s3_file_name = migration_audit_folder_path + 'audit_log_src_to_s3_' + db_name.lower() + '_' + table_name.lower() + '.csv'
...
def upload_table_to_S3_with_hook(Source_System_Name, bucket_name, database, table_name, Task_id, file_delim = '|', mysql_direct_to_file = False , exclude_columns = False, exclude_cols = [], **kwargs):
    ...
    #setting up connections and selecting audit table to help distribute audit inserts
    hook = airflow.hooks.S3_hook.S3Hook(s3_connection_id)
    snowflake_warehouse = distribute_warehouses()
    con = mysql_connection(mysql_username, mysql_password, mysql_hostname, mysql_port)
    sf_con = snowflake_connection(snowflake_username , snowflake_password, snowflake_account, snowflake_database, snowflake_warehouse, snowflake_stage_schema)
    ...
    for chunk_num in range(0, num_chunks):
        if date_field == 'NONEXISTENT':
            audit_df = start_audit_source_to_s3(Source_System_Name, date_field, database, table_name, Task_id, con, sf_con, audit_table = audit_table)
            ...
            audit_df = start_audit_source_to_s3(Source_System_Name, date_field, database, table_name, Task_id, con, sf_con, cdc_min_unix = query_chunks_df['min_date'][chunk_num], cdc_max_unix = query_chunks_df['max_date'][chunk_num], audit_table = audit_table)
        ...
        end_audit(database, table_name, sf_con, Task_id,  inserted_rows = row_count, sf_sql_statement = None, audit_table = audit_table)
...
def upload_to_snowflake(database, table_name, Task_id, Prev_task_id, field_delim = '|', **kwargs):
    ...
    audit_table = kwargs['ti'].xcom_pull(task_ids='select_audit_table_' + database + '_' + table_name)
    logging.info('selected table: ' + audit_table)
    audit_df = start_audit_snowflake(database, table_name, Task_id, Prev_task_id, con, audit_table = audit_table)
    ...
    if file_list == None: 
        ...
        end_audit(database, table_name, con, Task_id, audit_table = audit_table)
        con.close()
        con.close()
        gc.collect()
        return
    ...
    if len(file_list_slim) == 0:
        ...
        end_audit(database, table_name, con, Task_id, sf_sql_statement = copy, audit_table = audit_table)
    con.close()
...
#Cleans up large files on the EC2 instance to avoid the hard disk from reaching disk limits
def clean_logs(include_airflowdb=True):
    #Cleans out scheduler logs
    open(scheduler_log_file_path + 'airflow-scheduler.log', 'w').close()
    open(scheduler_log_file_path + 'airflow-scheduler.out', 'w').close()
    import shutil 
    from os import listdir
    hook = airflow.hooks.S3_hook.S3Hook(s3_connection_id)
    sched_path = '/root/airflow/logs/scheduler'
    d = listdir(sched_path)
    for dir_list in d:
        path = sched_path + '/' + dir_list + '/'
        file_list = listdir(path)
        for file in file_list:
            hook.load_file(path + file, key = 'airflow_logs/scheduler/' + dir_list + '/' + file, bucket_name = s3_bucket_name, replace=True)
    d.remove('latest')
    for folder in d:
        shutil.rmtree(sched_path + '/' + folder)
    
    #cleans out airflow database 
    if include_airflowdb==True:
        airflow_con = BaseHook.get_connection('airflow_db')
        con = mysql_connection(airflow_con.login , airflow_con.password , airflow_con.host, airflow_con.port)
        statement1 = ("Delete from airflow.log where dttm <= DATE(NOW() - INTERVAL %s DAY + INTERVAL 0 SECOND)"
                      % (airflow_db_log_lifepan)
                      )
        opt_statement1 = "OPTIMIZE TABLE airflow.log"
        con.execute(statement1)
        con.execute(opt_statement1)
        
        statement2 = ("Delete from airflow.task_instance where execution_date <= DATE(NOW() - INTERVAL '%s' DAY + INTERVAL 0 SECOND)"
                      % (airflow_db_log_lifepan)
                      )
        opt_statement2 = "OPTIMIZE TABLE airflow.task_instance"
        con.execute(statement2)
        con.execute(opt_statement2)
...
one_dag =  DAG(parent_dag_name, default_args=default_args, schedule_interval='@once', concurrency = 22) #in production, need to update this to run once daily (add various dags and set variables in Airflow?)
...
database_list = ['transaction_samhsp', 'transaction_scocar'] # 
