...
excluded_tables = ['web_service_request_log_part', 'web_service_request_log', 'event_queue_id', 'ct_config_version_cache'] #list of tables we dont want to migrate
...
def get_cdc_min_date(db_name, table_name, Task_id, connection, use_unix_format = False):
    AirflowTask = Task_id + db_name + '_' + table_name

    if use_unix_format == True:
        sql_statement = "SELECT IFNULL(MAX(CDCMAX_UNIX),0) AS CDCMIN FROM US_RAW.A_UTILITY.AUDIT_TASK_TABLE where AIRFLOW_TASK = '" + AirflowTask + "' and success = 'Y'"
        cdc_min_date = pd.read_sql_query(sql_statement, connection)
        cdc_min_date = cdc_min_date.iloc[0,0]
    else:
        sql_statement = "SELECT IFNULL(MAX(CDCMAX_TIMESTAMP),'1900-01-01') AS CDCMIN FROM US_RAW.A_UTILITY.AUDIT_TASK_TABLE where AIRFLOW_TASK = '" + AirflowTask + "' and success = 'Y'"
        cdc_min_date = pd.read_sql_query(sql_statement, connection)
...
#Fetches the the snowflake query id and row counts loaded to snowflake table from S3
...
def start_audit_source_to_s3(Source_System_Name, date_field, db_name, table_name, Task_id, source_connection, sf_connection
    ...
    if cdc_min_unix == None:
        #cdc_min_timestamp = get_cdc_min_date(db_name, table_name, Task_id, sf_connection)
        cdc_min_unix = get_cdc_min_date(db_name, table_name, Task_id, sf_connection, use_unix_format = True)
    ...
    df.to_sql('AUDIT_TASK_TABLE', schema = 'A_UTILITY', con = sf_connection, if_exists = 'append', index = False)
    return df
...
def start_audit_snowflake(db_name, table_name, Task_id, Prev_task_id, sf_connection):
...
def end_audit(db_name, table_name, sf_connection, Task_id, inserted_rows = 0, sf_sql_statement = None):
    end_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    AirflowTask = Task_id + db_name + '_' + table_name
    max_task_key = pd.read_sql_query("Select max(PROCESS_TASK_KEY) FROM US_RAW.A_UTILITY.AUDIT_TASK_TABLE WHERE AIRFLOW_TASK = '" + AirflowTask + "'", sf_connection).iloc[0,0]
    ...
    update = ("UPDATE US_RAW.A_UTILITY.AUDIT_TASK_TABLE SET "
          "END_DATETIME = '%s'"
          ...
          ";"
          %(end_time, sf_query_id, inserted_rows, max_task_key)
    )
...
def get_chunk_df( database, table_name, chunk_size = 1000000, time_divide = 86000):
    ...
    cdc_min = get_cdc_min_date(database, table_name, 'upload_to_S3_task_', sf_con, use_unix_format = True)
    sql = "Select  floor(" + date_field + "/" + str(time_divide) + ") day, count(*) row_count from " + database + '.' + table_name + " WHERE " + date_field + " > " + str(cdc_min) + " group by floor(" + date_field + "/" + str(time_divide) + ") order by floor(" + date_field + "/" + str(time_divide) + ")"
    row_counts = pd.read_sql_query(sql, con)
    ...
    if date_ranges_df.shape[0] == 0:
        ...
        date_ranges_df.loc[0,'max_date'] = get_cdc_max_date(date_field, database, table_name, con, use_unix_format = True)
    ...
    for chunk_num in range(0, num_chunks):
        ...
        if mysql_direct_to_file == True:
            ...
            for file_chunk in pd.read_sql_query(sql_statement, con, chunksize = max_rows_per_text_file):
                ...
                s3_file_name = generate_s3_file_name(database, table_name, file_iterator = file_count, cdc_min_unix = audit_df['CDCMIN_UNIX'][0], cdc_max_unix = audit_df['CDCMAX_UNIX'][0]) 
...
def upload_to_snowflake(database, table_name, Task_id, Prev_task_id, field_delim = '|'):
    ...
    s3_file_base_name =  database + '_' + table_name
    ...
    hook = airflow.hooks.S3_hook.S3Hook(s3_connection_id)
    file_list = hook.list_keys(bucket_name = s3_bucket_name)
    if file_list == None: 
    ...
    if len(file_list_slim) == 0:
        ...
        con.execute(copy)
        for i in file_list_slim:
            hook.copy_object(source_bucket_key = i, dest_bucket_key =  'success/' + i, source_bucket_name = s3_bucket_name,  dest_bucket_name = s3_bucket_name)
...
database_list = ['transaction_alhsag', 'transaction_alhspr', 'transaction_alhswo', 'transaction_altush', 'transaction_ambcln']
